...
export class RegistryQueue {
  ...
  constructor(connectionString: string) {
    this.boss = new PgBoss({
      ...
      migrate: process.env.REGISTRY_QUEUE_MIGRATE !== "false",
      createSchema: process.env.REGISTRY_QUEUE_MIGRATE !== "false",
    });
  ...
  async enqueueRun(job: RegistryRunJob, priority = 0): Promise<string> {
    ...
    if (!id) throw new Error(`A Registry queue job already exists for run ${job.runId}.`);
    return id;
  }
  ...
  async resume(jobId: string): Promise<void> {
    const boss = await this.start();
    await boss.resume(REGISTRY_QUEUE_NAME, jobId);
  }
